...
function begin_play(){
		...
		var p1 = isEmpty(document.getElementById('player1_id').value);
		var p2 = isEmpty(document.getElementById('player2_id').value);
		if(p1||p2){
			alert("Two palyer game. Both field are mandatory");
			return;
		}
		this.started = true;
...
function play() {
	/*
	check if the game is start and if so alert the player
	make a regex that check for vcaled move and make sure spot is free
	(move_text_id)
	figure out whoes move it and call toggle_move
	board_state[num] is the locations
	*/
	if(!game_started()){
		alert("The game has not started.");
		document.getElementById('move_text_id').innerHTML = "";
		return;
	}
	var input = document.getElementById('move_text_id').value
	if(!(input.match(/^[A-C][1-3]$/g))|| (document.getElementById(input).innerHTML != input)){
		alert("Input is not valid");
		document.getElementById('move_text_id').innerHTML = "";
		return;
	}
	var player = whose_move();
	var locations = getIndexFromSet(table_ids,input);
	if(player){
		document.getElementById(input).innerHTML = "X";
		board_state[locations] = player;
	...
	var case_1 = (board_state[0] == player) && (board_state[1] == player) && (board_state[2] == player) && (board_state[0] != -1) && (board_state[1] != -1) && (board_state[2] != -1);
	var case_2 = (board_state[3] == player) && (board_state[4] == player) && (board_state[5] == player) && board_state[3] != -1 && board_state[4] != -1 && board_state[5] != -1;
	var case_3 = (board_state[6] == player) && (board_state[7] == player) && (board_state[8] == player) && board_state[6] != -1 && board_state[7] != -1 && board_state[8] != -1;
	var case_4 = (board_state[0] == player) && (board_state[3] == player) && (board_state[6] == player) && board_state[0] != -1 && board_state[3] != -1 && board_state[6] != -1;
	var case_5 = (board_state[1] == player) && (board_state[4] == player) && (board_state[7] == player) && board_state[1] != -1 && board_state[4] != -1 && board_state[7] != -1;
	var case_6 = (board_state[2] == player) && (board_state[5] == player) && (board_state[8] == player) && board_state[2] != -1 && board_state[5] != -1 && board_state[8] != -1;
	var case_7 = (board_state[0] == player) && (board_state[4] == player) && (board_state[8] == player) && board_state[0] != -1 && board_state[4] != -1 && board_state[8] != -1;
	var case_8 = (board_state[2] == player) && (board_state[4] == player) && (board_state[6] == player) && board_state[2] != -1 && board_state[4] != -1 && board_state[6] != -1;
